...
def taxonomy_level_index(s):
    ...
    if (
        s in non_taxonomic_prediction_strings
        or s in non_taxonomic_prediction_short_strings
    ):
        return 0
...
def read_sequences_from_download_bundle(download_folder):
    ...
    sequence_list_files = [fn for fn in sequence_list_files if fn == "sequences.csv"]
    assert len(sequence_list_files) == 1, "Could not find sequences.csv in {}".format(
        download_folder
    )
...
def read_images_from_download_bundle(download_folder):
    ...
    image_list_files = [
        fn
        for fn in image_list_files
        if fn.startswith("images_") and fn.endswith(".csv")
    ]
    image_list_files = [os.path.join(download_folder, fn) for fn in image_list_files]
    ...
    for image_list_file in image_list_files:
        ...
        print(
            "Reading images from list file {}".format(os.path.basename(image_list_file))
        )
    ...
    print(
        "Found {} rows in {} deployments".format(
            len(image_id_to_image_records), len(deployment_ids)
        )
    )
...
def find_images_in_identify_tab(
    download_folder_with_identify, download_folder_excluding_identify
):
    """
    ...
    image_id_to_image_records_with_identify = read_images_from_download_bundle(
        download_folder_with_identify
    )
    image_id_to_image_records_excluding_identify = read_images_from_download_bundle(
        download_folder_excluding_identify
    )

    ##%% Find images that have not been identified

    all_image_ids_with_identify = set(image_id_to_image_records_with_identify.keys())
    all_image_ids_excluding_identify = set(
        image_id_to_image_records_excluding_identify.keys()
    )

    image_ids_in_identify_tab = all_image_ids_with_identify.difference(
        all_image_ids_excluding_identify
    )

    assert len(image_ids_in_identify_tab) == len(all_image_ids_with_identify) - len(
        all_image_ids_excluding_identify
    )

    print(
        "Found {} images with identify, {} in identify tab, {} excluding".format(
    ...
    print(
        "Found {} records for {} unique images in {} deployments".format(
...
def write_download_commands(
    ...
    if isinstance(image_records_to_download, dict):
        ...
        return write_download_commands(
            all_image_records,
    ...
    if download_command_file_base is None:
        download_command_file_base = os.path.join(
            download_dir_base, "download_wi_images.sh"
        )
    ...
    for image_record in tqdm(image_records_to_download):
        ...
        command = 'gcloud storage cp --no-clobber "{}" "./{}"'.format(
            url, relative_path
        )
        commands.append(command)

    print(
        "Generated {} commands for {} image records".format(
    ...
    for i_script in range(0, n_download_workers):
        download_command_file = insert_before_extension(
            download_command_file_base, str(i_script).zfill(2)
        )
...
non_taxonomic_prediction_short_strings = [
    ";".join(s.split(";")[1:-1]) for s in non_taxonomic_prediction_strings
]
...
def push_results_for_images(
    payload, headers, url=process_cv_response_url, verbose=False
):
    """
    ...
    if verbose:
        print("Sending header {} to URL {}".format(headers, url))
    ...
    if response.status_code in (200, 201):
        if verbose:
            print(
                "Successfully pushed results for {} images".format(
...
def parallel_push_results_for_images(
    ...
    if n_workers == 1:
        ...
        for payload in payloads:
            results.append(
                push_results_for_images(
                    payload, headers=headers, url=url, verbose=verbose
                )
            )
        return results
        ...
        try:
            ...
            print("Created a {} pool of {} workers".format(pool_string, n_workers))
...
def generate_payload_for_prediction_string(
    ...
    assert is_valid_prediction_string(
        prediction_string
...
def validate_payload(payload):
    ...
    for prediction in payload["predictions"]:
        ...
        assert "ignore_data_file_checks" in prediction and isinstance(
            prediction["ignore_data_file_checks"], bool
        )
        assert (
            "prediction" in prediction
            and isinstance(prediction["prediction"], str)
            and len(prediction["prediction"].split(";")) == 7
        )
        assert "prediction_score" in prediction and isinstance(
            prediction["prediction_score"], float
        )
        assert "model_version" in prediction and isinstance(
            prediction["model_version"], str
        )
        assert (
            "data_file_id" in prediction
            and isinstance(prediction["data_file_id"], str)
            and len(prediction["data_file_id"]) == 36
        )
        assert "classifications" in prediction and isinstance(
            prediction["classifications"], dict
        )
        classifications = prediction["classifications"]
        assert "classes" in classifications and isinstance(
            classifications["classes"], list
        )
        assert "scores" in classifications and isinstance(
            classifications["scores"], list
        )
        ...
        for detection in prediction["detections"]:
            ...
            assert "label" in detection and detection["label"] in (
                "animal",
                "person",
                "vehicle",
            )
            assert (
                "conf" in detection
                and isinstance(detection["conf"], float)
                and detection["conf"] >= 0
                and detection["conf"] <= 1.0
            )
            assert (
                "bbox" in detection
                and isinstance(detection["bbox"], list)
...
blank_payload = generate_blank_prediction_payload(
...
def is_human_classification(prediction_string):
    ...
    """
    return (
        prediction_string == human_prediction_string
        or "homo;sapiens" in prediction_string
    )
...
def generate_md_results_from_predictions_json(
    ...
    for im_in in predictions:
        ...
        # Copy over extra metadata fields if present
        for meta_field in [
            "seq_id",
            "datetime",
            "date",
            ...
            "admin1_region",
            "smoothed_class",
            "smoothed_class_score",
        ]:
            if meta_field in im_in:
                im_out[meta_field] = im_in[meta_field]
        ...
        if "failures" in im_in:
            ...
            if "prediction" in im_in:
                ...
                class_to_assign = None
                im_out["top_classification_common_name"] = (
                    top_classification_common_name
                )
                class_to_assign = im_in["prediction"]
                if convert_homo_species_to_human and class_to_assign.endswith(
                    "homo species"
                ):
    ...
    if base_folder is not None:
        if base_folder_replacements == 0:
            print(
                "Warning: you supplied {} as the base folder, but I made zero replacements".format(
                    base_folder
                )
            )
...
def generate_predictions_json_from_md_results(
    md_results_file, predictions_json_file, base_folder=None
):
    """
...
def generate_instances_json_from_folder(
    ...
    if tokens_to_ignore is not None:
        ...
        print(
            "After ignoring {} tokens, kept {} of {} images".format(
                len(tokens_to_ignore),
                len(image_files_abs),
                n_images_before_ignore_tokens,
            )
        )
...
def split_instances_into_n_batches(instances_json, n_batches, output_files=None):
    ...
    if output_files is not None:
        assert (
            len(output_files) == n_batches
        ), "Expected {} output files, received {}".format(n_batches, len(output_files))
    else:
        output_files = []
...
def load_md_or_speciesnet_file(fn, verbose=True):
    ...
    if "predictions" in detector_output:
        ...
        try:
            ...
            if verbose:
                print(
                    "Warning: error removing temporary .json {}".format(
                        temp_results_file
                    )
                )
...
def validate_predictions_file(fn, instances=None, verbose=True):
    ...
    if verbose:
        print(
            "Read predictions for {} images, with {} failure(s)".format(
    ...
    if instances is not None:
        ...
        if isinstance(instances, str):
            if os.path.isdir(instances):
                ...
                raise ValueError(
                    "Could not find instances file/folder {}".format(instances)
                )
        ...
        assert len(instances) == len(
            predictions
        ), "{} instances expected, {} found".format(len(instances), len(predictions))
...
def find_geofence_adjustments(ensemble_json_file, use_latin_names=False):
    ...
    for prediction in tqdm(predictions):
        ...
        if (
            ...
            and prediction["failures"] is not None
            and len(prediction["failures"]) > 0
        ):
            continue
        ...
        if "geofence" in prediction["prediction_source"]:
            ...
            classification_taxonomy_string = prediction["classifications"]["classes"][0]
            ...
            input_name = (
                classification_binomial_name
                if use_latin_names
                else classification_common_name
            )
            output_name = (
                prediction_binomial_name if use_latin_names else prediction_common_name
            )
...
def initialize_taxonomy_info(taxonomy_file, force_init=False, encoding="cp1252"):
    ...
    print(
        "Created {} records in taxonomy_string_to_taxonomy_info".format(
            len(taxonomy_string_to_taxonomy_info)
        )
    )
    print(
        "Created {} records in common_name_to_taxonomy_info".format(
            len(common_name_to_taxonomy_info)
        )
    )
...
def _generate_csv_rows_to_block_all_countries_except(species_string, block_except_list):
    """
    ...
    assert is_valid_taxonomy_string(
        species_string
    ...
    assert (
        taxonomy_string_to_geofencing_rules is not None
    ), "Initialize geofencing prior to species lookup"
    assert (
        taxonomy_string_to_taxonomy_info is not None
    ), "Initialize taxonomy lookup prior to species lookup"

    geofencing_rules_this_species = taxonomy_string_to_geofencing_rules[species_string]
    ...
    rows = generate_csv_rows_for_species(
        species_string,
...
def initialize_geofencing(geofencing_file, country_code_file, force_init=False):
    ...
    if (
        (country_to_country_code is not None)
        and (country_code_to_country is not None)
        and (taxonomy_string_to_geofencing_rules is not None)
        and (not force_init)
    ):
        return
    ...
    for species_string in taxonomy_string_to_geofencing_rules.keys():
        ...
        if len(species_rules.keys()) > 1:
            print(
                "Warning: taxon {} has both allow and block rules".format(
                    species_string
                )
            )
...
def export_geofence_data_to_csv(csv_fn, include_common_names=True):
    ...
    for taxon in tqdm(all_taxa):
        for region in all_regions:
            ...
            if len(tokens) > 1:
                state_code = tokens[1]
            allowed = species_allowed_in_country(
                species=taxon,
    ...
    print(
        "Allowed {} of {} combinations".format(
            n_allowed, len(all_taxa) * len(all_regions)
        )
    )
...
if False:
    ...
    from megadetector.utils.wi_utils import (
        initialize_geofencing,
        initialize_taxonomy_info,
    )  # noqa
    from megadetector.utils.wi_utils import (
        _species_string_to_canonical_species_string,
    )  # noqa
    from megadetector.utils.wi_utils import generate_csv_rows_for_species  # noqa
    from megadetector.utils.wi_utils import (
        _generate_csv_rows_to_block_all_countries_except,
    )  # noqa
    ...
    geofencing_file = os.path.join(
        model_base, "crop", "geofence_release.2025.02.27.0702.json"
    )
    ...
    encoding = None
    taxonomy_file = os.path.join(model_base, "taxonomy_mapping.json")
    ...
    rows = generate_csv_rows_for_species(
        species_string=taxonomy_string_short,
        allow_countries=None,
    ...
    rows = _generate_csv_rows_to_block_all_countries_except(
        species_string, block_except_list
    )
    ...
    for species in all_species:
        ...
        print(
            "{} ({}) for {}{}: {}".format(
                taxonomy_info["common_name"], species, country, state_string, allowed
            )
        )
